...
class RFF(nn.Module):
    ...
    def training_forward(self, x: torch.Tensor, n: int):
        ...
        for current_depth in range(self.depth):
            ...
            current_weights = self.rw[n][platform:next_platform] # ((self.n_leaves-1), in_features)    
            current_biases = self.rb[n][platform:next_platform]   # ((self.n_leaves-1), 1)
        ...
        for n in range(self.n_trees):
            ...
            for i in range(leaves.shape[0]):
                ...
                logits = torch.matmul(
                    ...
                    self.w1s[n][leaf_index]                # (self.in_features, self.leaf_width)
                )                                               # (1, self.leaf_width)
                logits += self.b1s[n][leaf_index].unsqueeze(-2)    # (1, self.leaf_width)
                activations = self.activation(logits)           # (1, self.leaf_width)
                new_logits[i] = torch.matmul(
                    activations,
                    self.w2s[n][leaf_index]
