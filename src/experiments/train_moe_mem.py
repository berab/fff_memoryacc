...
from utils.nn import moe_train_epoch, eval_model, moe_train_epoch_mem
...
class TrainMoEMEM(BaseTrainExp):
    def __init__(self):
        super().__init__()  # Initialize BaseExp
        self.exp_name = "TrainMoEMEM"
    ...
    def run_exp(self) -> dict:
        ...
        for epoch in range(self.epochs+self.dist_warmup+self.mem_tune):
            if (epoch < self.dist_warmup) or (self.mem_tune > 0 and epoch < self.epochs): # Some epochs of pretraining without dist reg.
                train_loss, train_acc, reg_loss, entropy_loss, mem_loss = moe_train_epoch(self.model, self.optim, self.loader.train, self.criterion, epoch, self.device, self.reg_alpha, self.entropy_alpha)
            else:
                train_loss, train_acc, reg_loss, entropy_loss, mem_loss = moe_train_epoch_mem(self.model, self.optim, self.loader.train, self.criterion, epoch, self.device, self.reg_alpha, self.entropy_alpha, n_mem1, self.mem_alpha, mem1_experts)
