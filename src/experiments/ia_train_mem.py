...
from utils.nn import ia_train_epoch, eval_model, ia_train_epoch_mem
from utils.fff_stats import get_leaves, get_leaf_stats, get_partition_count

MCU_CONFIG = {
    "apollo": [384, 1000], # TCM, SRAM
    "stm": [94, 1000], # SRAM, FLASH
}

class IATrainMEM(BaseTrainExp):
    def __init__(self, alpha: float = 0.0):
        super().__init__()  # Initialize BaseExp
        self.exp_name = "IATrainMEM"
        self.a_alpha = alpha
    ...
    def run_exp(self) -> dict:
        ...
        if n_mem1 == 0:
            ...
            return metrics
        mlflow.log_metric("n_mem1", n_mem1)
        # Training
        val_leaves, all_val_leaf_stats = [], []
        mem1_leaves = None
        assert (self.dist_warmup > 0 or self.mem_tune > 0), "Both dist_warmup and mem_tune is lower or equal to zero. Do some calibration, this is the point of the experiment."
        assert not (self.dist_warmup > 0 and self.mem_tune > 0), "Both dist_warmup and mem_tune is lower or equal to zero. Do some calibration, this is the point of the experiment."
        for epoch in range(self.epochs+self.dist_warmup+self.mem_tune):
            if (epoch < self.dist_warmup) or (self.mem_tune > 0 and epoch < self.epochs): # Some epochs of pretraining without dist reg.
                train_loss, train_acc, reg_loss, entropy_loss, mem_loss = ia_train_epoch(self.model, self.optim, self.loader.train, self.criterion, epoch, self.device, self.reg_alpha, self.entropy_alpha, self.a_alpha)
            else:
                train_loss, train_acc, reg_loss, entropy_loss, mem_loss = ia_train_epoch_mem(self.model, self.optim, self.loader.train, self.criterion, epoch, self.device, self.reg_alpha, self.entropy_alpha, self.a_alpha, n_mem1, self.mem_alpha, mem1_leaves)
