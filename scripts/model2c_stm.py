...
# Lets start easy: dont divide the leaves into neurons and share into memories. keep a whole leaves in a memory (SRAM and FLASH). Do that later?
...
def write_config(out_file, depth, leaf_width, n_leaves_in_sram, remain_leaves):
    # Model Config & Params
...
def write_weights(state_dict, out_file, n_leaves_in_sram):
    # Model Config & Params
    with open(out_file, "w") as f:
        # Params
        ...
        # SRAM
        lw1_1, lb1_1 = lw1[:n_leaves_in_sram], lb1[:n_leaves_in_sram]
        lw2_1, lb2_1 = lw2[:n_leaves_in_sram], lb2[:n_leaves_in_sram]
...
def main(state_dict_file: str):
    ...
    remain_sram -= cal_nodes_size(depth)
    ...
    write_config(OUT_DIR/config_filename, depth, leaf_width, n_leaves_in_sram, remain_leaves)
    write_weights(state_dict, OUT_DIR/weights_filename, n_leaves_in_sram)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config-name', default="mnist_a08_h8_d4_l4", help="Configuration name (e.g., 'mnist_d4_l16')")
